...
#'
#' @details
#' The `SB.sPLS-DRCOX` function performs a single-block sparse partial least squares deviance residual
...
#'
#' In summary, the `SB.sPLS-DRCOX` function offers a robust approach for survival analysis with
...
#' Y <- Y_multiomic
#' sb.splsdrcox_penalty(X_train, Y_train, n.comp = 2, penalty = 0.5, x.center = TRUE, x.scale = TRUE)

sb.splsdrcox_penalty <- function (X, Y,
  ...
  lst_sb.spls <- list()
  for(b in names(Xh)){
    lst_sb.spls[[b]] <- splsdrcox_penalty(X = Xh[[b]], Y = Yh, n.comp = n.comp, penalty = penalty,
  ...
  # invisible(gc())
  return(sb.splsdrcox_penalty_class(list(X = list("data" = if(returnData) X_norm else NA,
                                          "scores" = all_scores,
                                 ...
                                 class = pkg.env$sb.splsdrcox_penalty,
                                 time = time)))
}
...
#' @description This function performs cross-validated sparse partial least squares single block for splsdrcox.
...
#'
#' @details
#' The `cv.sb.splsdrcox` function performs cross-validation for the single-block sparse partial least
...
#'
#' In summary, the `cv.sb.splsdrcox` function offers a robust approach for hyperparameter tuning and
...
#'
#' @return Instance of class "Coxmos" and model "cv.SB.sPLS-DRCOX".
...
cv.sb.splsdrcox_penalty <- function(X, Y,
  ...
  lst_model <- get_Coxmos_models2.0(method = pkg.env$sb.splsdrcox_penalty,
                                    X_train = X, Y_train = Y,
  ...
  #   time <- difftime(t2,t1,units = "mins")
  #   if(return_models){
  #     return(cv.sb.splsdrcox_penalty_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = lst_model, pred.method = pred.method, opt.comp = NULL, opt.penalty = NULL, plot_AIC = NULL, plot_C.Index = NULL, plot_I.BRIER = NULL, plot_AUC = NULL, class = pkg.env$cv.sb.splsdrcox_penalty, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  #   }else{
  #     return(cv.sb.splsdrcox_penalty_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = NULL, pred.method = pred.method, opt.comp = NULL, opt.penalty = NULL, plot_AIC = NULL, plot_C.Index = NULL, plot_I.BRIER = NULL, plot_AUC = NULL, class = pkg.env$cv.sb.splsdrcox_penalty, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  #   }
  # }
  ...
  if(TRUE){ #compute always BRIER SCORE
    ...
    lst_df <- get_COX_evaluation_BRIER_sPLS(comp_model_lst = lst_model$comp_model_lst,
                                            ...
                                            w_I.BRIER = w_I.BRIER, method.train = pkg.env$sb.splsdrcox_penalty, PARALLEL = FALSE, n_cores = NULL, verbose = verbose)
  ...
  if(w_AUC!=0){
    ...
    lst_df <- get_COX_evaluation_AUC_sPLS(comp_model_lst = lst_model$comp_model_lst,
                                          ...
                                          max.ncomp = max.ncomp, penalty.list = penalty.list, n_run = n_run, k_folds = k_folds,
                                          MIN_AUC_INCREASE = MIN_AUC_INCREASE, MIN_AUC = MIN_AUC, MIN_COMP_TO_CHECK = MIN_COMP_TO_CHECK,
                                          w_AUC = w_AUC, method.train = pkg.env$sb.splsdrcox_penalty, PARALLEL = FALSE, n_cores = NULL, verbose = verbose)
  ...
  # invisible(gc())
  if(return_models){
    return(cv.sb.splsdrcox_penalty_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = lst_model, pred.method = pred.method, opt.comp = best_model_info$n.comps, opt.penalty = best_model_info$penalty, plot_AIC = ggp_AIC, plot_C.Index = ggp_C.Index, plot_I.BRIER = ggp_IBS, plot_AUC = ggp_AUC, class = pkg.env$cv.sb.splsdrcox_penalty, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  }else{
    return(cv.sb.splsdrcox_penalty_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = NULL, pred.method = pred.method, opt.comp = best_model_info$n.comps, opt.penalty = best_model_info$penalty, plot_AIC = ggp_AIC, plot_C.Index = ggp_C.Index, plot_I.BRIER = ggp_IBS, plot_AUC = ggp_AUC, class = pkg.env$cv.sb.splsdrcox_penalty, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  }
}
...
sb.splsdrcox_penalty_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$sb.splsdrcox_penalty)
  return(model)
}

cv.sb.splsdrcox_penalty_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$cv.sb.splsdrcox_penalty)
  return(model)
}
