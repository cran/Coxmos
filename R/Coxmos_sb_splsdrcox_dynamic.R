...
#'
#' @details
#' The `SB.sPLS-DRCOX-Dynamic` function performs a single-block sparse partial least squares deviance residual
...
#'
#' In summary, the `SB.sPLS-DRCOX-Dynamic` function offers a robust approach for survival analysis with
...
sb.splsdrcox <- function(X, Y,
  ...
  for(b in names(Xh)){
    ...
    lst_sb.spls[[b]] <- splsdrcox(X = Xh[[b]], Y = Yh, n.comp = n.comp, vector = aux_vector,
  ...
  # invisible(gc())
  return(sb.splsdrcox_dynamic_class(list(X = list("data" = if(returnData) X_norm else NA,
                                         ...
                                         class = pkg.env$sb.splsdrcox_dynamic,
                                         time = time)))
}
...
#'
#' @return Instance of class "Coxmos" and model "cv.SB.sPLS-DRCOX-Dynamic".
...
cv.sb.splsdrcox <- function(X, Y,
  ...
  lst_model <- get_Coxmos_models2.0(method = pkg.env$sb.splsdrcox_dynamic,
                                   X_train = X, Y_train = Y,
  ...
  if(all(is.null(df_results_evals))){
    ...
    time <- difftime(t2,t1,units = "mins")
    if(return_models){
      return(cv.sb.splsdrcox_dynamic_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = lst_model, pred.method = pred.method, opt.comp = NULL, opt.nvar = NULL, plot_AIC = NULL, plot_C.Index = NULL, plot_I.BRIER = NULL, plot_AUC = NULL, class = pkg.env$cv.sb.splsdrcox_dynamic, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
    }else{
      return(cv.sb.splsdrcox_dynamic_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = NULL, pred.method = pred.method, opt.comp = NULL, opt.nvar = NULL, plot_AIC = NULL, plot_C.Index = NULL, plot_I.BRIER = NULL, plot_AUC = NULL, class = pkg.env$cv.sb.splsdrcox_dynamic, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
    }
  }
  ...
  # invisible(gc())
  if(return_models){
    return(cv.sb.splsdrcox_dynamic_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = lst_model, pred.method = pred.method, opt.comp = best_model_info$n.comps, opt.nvar = opt_n.var, plot_AIC = ggp_AIC, plot_C.Index = ggp_C.Index, plot_I.BRIER = ggp_IBS, plot_AUC = ggp_AUC, class = pkg.env$cv.sb.splsdrcox_dynamic, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  }else{
    return(cv.sb.splsdrcox_dynamic_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = NULL, pred.method = pred.method, opt.comp = best_model_info$n.comps, opt.nvar = opt_n.var, plot_AIC = ggp_AIC, plot_C.Index = ggp_C.Index, plot_I.BRIER = ggp_IBS, plot_AUC = ggp_AUC, class = pkg.env$cv.sb.splsdrcox_dynamic, lst_train_indexes = lst_train_indexes, lst_test_indexes = lst_test_indexes, time = time)))
  }
}
...
sb.splsdrcox_dynamic_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$sb.splsdrcox_dynamic)
  return(model)
}

cv.sb.splsdrcox_dynamic_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$cv.sb.splsdrcox_dynamic)
  return(model)
}
