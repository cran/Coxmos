...
#' Cox analysis (sPLS-ICOX) using the optimal components and variables identified in a previous
#' cross-validation process. It builds the final model based on the selected hyperparameters.
#'
#' @details
#' The `isb.splsicox` function fits a single-block sPLS-ICOX model using the input data
...
#' @param cv.isb Instance of class "Coxmos" and model "cv.iSB.sPLS-ICOX-Dynamic". Used to retrieve the
...
#'
#' @return An object of class "Coxmos" and model "isb.splsicox", containing:
#' \itemize{
...
isb.splsicox <- function(X, Y,
        ...
        # Check class of CV
        if(!class(cv.isb) %in% pkg.env$model_class || !attr(cv.isb, "model") %in% pkg.env$cv.isb.splsicox){
          stop(paste0("Cross validation object 'cv.isb' must belong to '", pkg.env$cv.isb.splsicox, "' class."))
        }
        ...
        for(b in names(Xh)){
          ...
          message(paste0("Creating ", pkg.env$isb.splsicox ," model for block: ", b, "..."))
          t1 <- Sys.time()

          cv.splsicox_res <- cv.isb$list_cv_spls_models[[b]]

          lst_sb.spls[[b]] <- splsicox(X = Xh[[b]],
                                      Y = Yh,
...
#' sPLS-ICOX-Dynamic. It returns the optimal number of components and the optimal sparsity penalty value based
...
#'
#' @details
#' The `cv.isb.splsicox` function performs cross-validation for the single-block sparse partial least
...
#' (default: 8).
#' @param penalty.list Numeric vector. Vector of penalty values. Penalty for sPLS-DACOX. If
...
#'
#' @return An instance of class "Coxmos" and model "cv.SB.sPLS-ICOX-Dynamic", containing:
#' \itemize{
...
cv.isb.splsicox <- function(X, Y,
  ...
  for(b in names(Xh)){
    ...
    message(paste0("\nRunning cross validation ", pkg.env$isb.splsicox ," for block: ", b, "\n"))
    t1 <- Sys.time()

    lst_cv.sb.spls[[b]] <- cv.splsicox(X = Xh[[b]], Y = Yh,
                                       ...
                                       #y.scale = y.scale, y.center = y.center,
                                       remove_near_zero_variance = remove_near_zero_variance, remove_zero_variance = FALSE, toKeep.zv = NULL,
                                       remove_variance_at_fold_level = remove_variance_at_fold_level,
                                       remove_non_significant = remove_non_significant,
  ...
  # invisible(gc())
  return(cv.isb.splsicox_class(list(X = list("data" = if(returnData) X_norm else NA,
                                                   "x.mean" = xmeans, "x.sd" = xsds),
                                          Y = list("data" = Yh,
                                                   "y.mean" = ymeans, "y.sd" = ysds),
...
cv.isb.splsicox_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$cv.isb.splsicox)
  return(model)
}
