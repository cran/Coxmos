...
#'
#' @return An object of class "Coxmos" and model "isb.splsdrcox", containing:
#' \itemize{
...
isb.splsdrcox_penalty <- function(X, Y,
                          cv.isb,
        ...
        # Check class of CV
        if(!class(cv.isb) %in% pkg.env$model_class || !attr(cv.isb, "model") %in% pkg.env$cv.isb.splsdrcox_penalty){
          stop(paste0("Cross validation object 'cv.isb' must belong to '", pkg.env$cv.isb.splsdrcox_penalty, "' class."))
        }
        ...
        for(b in names(Xh)){
          ...
          message(paste0("Creating ", pkg.env$isb.splsdrcox_penalty ," model for block: ", b, "..."))
          t1 <- Sys.time()

          cv.splsdrcox_res <- cv.isb$list_cv_spls_models[[b]]

          lst_sb.spls[[b]] <- splsdrcox_penalty(X = Xh[[b]],
                                        Y = Yh,
        ...
        return(isb.splsdrcox_class(list(X = list("data" = if(returnData) X_norm else NA,
                                      ...
                                      class = pkg.env$isb.splsdrcox_penalty,
                                      time = time)))
      }
...
cv.isb.splsdrcox_penalty <- function(X, Y,
  ...
  for(b in names(Xh)){
    ...
    message(paste0("\nRunning cross validation ", pkg.env$isb.splsdrcox_penalty ," for block: ", b, "\n"))
    t1 <- Sys.time()
  ...
  return(cv.isb.splsdrcox_class(list(X = list("data" = if(returnData) X_norm else NA,
                                          ...
                                          nzv = variablesDeleted,
                                          nz_coeffvar = variablesDeleted_cvar,
                                          class = pkg.env$cv.isb.splsdrcox_penalty,
                                          time = time)))
}
...
isb.splsdrcox_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$isb.splsdrcox_penalty)
  return(model)
}

cv.isb.splsdrcox_class = function(pls_model, ...) {
  model = structure(pls_model, class = pkg.env$model_class,
                    model = pkg.env$cv.isb.splsdrcox_penalty)
  return(model)
}
